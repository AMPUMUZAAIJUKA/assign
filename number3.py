...
if bfs_path:
    print(" Path for the Breath First Search from 'S' to 'G':", ' -> '.join(bfs_path))
else:
    print("No valid path ")
    ...
    while priority_queue:
        ...
        if current_node == goal:
            ...
            while previous_nodes[current_node] is not None:
                path.insert(0, current_node)  #  node at the beginning of the path
                current_node = previous_nodes[current_node]
            path.insert(0, start)  # start node at the beginning of the path
